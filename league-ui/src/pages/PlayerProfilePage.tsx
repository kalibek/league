...
import { usePlayer, usePlayerEvents } from '../hooks/usePlayers'
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {g.place > 0 && (
            <span style={{ fontSize: 11, color: '#94a3b8' }}>
              {t('playerProfile.place', { place: g.place })} ·{' '}
              {t('playerProfile.points', { points: g.points })}
            </span>
          )}
          {g.advances && (
            <span
    ...
    >
      ...
      >
        <div>
          ...
          >
            <p style={{ fontWeight: 700, color: 'var(--navy)', fontSize: 15, marginBottom: 3 }}>
              {ev.title}
            </p>
          </Link>
      ...
      {ev.ratingBefore != null && ev.ratingAfter != null && (
        ...
        >
          ...
          <span style={{ fontWeight: 700, color: 'var(--navy)' }}>
            {Math.round(ev.ratingBefore)}
          </span>
          <span style={{ color: '#cbd5e1' }}>→</span>
          <span style={{ fontWeight: 700, color: 'var(--navy)' }}>
            {Math.round(ev.ratingAfter)}
          </span>
          {ev.ratingDelta !== 0 && <RatingDelta delta={Math.round(ev.ratingDelta)} />}
        </div>
      )}
...
export function PlayerProfilePage() {
  ...
  const profileFields: { label: string; value: string | null | undefined }[] = prof
    ? [
        ...
        {
          label: t('playerProfile.grip'),
          value: prof.grip ? prof.grip.charAt(0).toUpperCase() + prof.grip.slice(1) : null,
        },
        {
          label: t('playerProfile.gender'),
          value: prof.gender ? prof.gender.charAt(0).toUpperCase() + prof.gender.slice(1) : null,
        },
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 3 }}>
              ±{Math.round(player.deviation)} RD
            </p>
            {lastDelta !== 0 && (
      ...
      >
        {!prof || !prof.isComplete ? (
          <p style={{ fontSize: 13, color: '#94a3b8', fontStyle: 'italic' }}>
            {t('playerProfile.profileNotFilled')}
          </p>
        ) : (
      ...
      >
        {t('playerProfile.eventHistory')}{' '}
        {total > 0 && <span style={{ fontWeight: 400 }}>({total})</span>}
      </h2>

      {events.length === 0 && !eventsLoading && (
        <p style={{ fontSize: 14, color: '#94a3b8', textAlign: 'center', padding: '32px 0' }}>
          {t('playerProfile.noEventsYet')}
        </p>
      )}
